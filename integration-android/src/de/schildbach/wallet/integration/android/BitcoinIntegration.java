...
public final class BitcoinIntegration
{
	...
	{
		final StringBuilder uri = new StringBuilder("worldcoin:");
		if (address != null)
			uri.append(address);
		if (amount != null)
	...
	{
		Toast.makeText(context, "No Worldcoin application found.\nPlease install Worldcoin Wallet.", Toast.LENGTH_LONG).show();

		final Intent marketIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=de.schildbach.wallet"));
		final Intent binaryIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("http://code.google.com/p/worldcoin-wallet/downloads/list"));
